...
import com.komentum.component.dto.CreateThemeRequest;
import com.komentum.component.repository.ColorStyleRepository;
import com.komentum.exception.ResourceNotFoundException;
import com.komentum.theme.domain.ThemeComponent;
import com.komentum.theme.domain.ThemeImage;
import com.komentum.theme.domain.ThemeImageId;
import com.komentum.theme.domain.ThemeStyle;
import com.komentum.theme.dto.ThemeComponentDto;
import com.komentum.theme.dto.ThemeImageDto;
import com.komentum.theme.dto.ThemeStyleDto;
import com.komentum.theme.dto.ThemeStyleRequest;
import com.komentum.theme.repository.ThemeComponentRepository;
import com.komentum.theme.repository.ThemeImageRepository;
import com.komentum.theme.repository.ThemeStyleRepository;
...
public class ThemeService {
  ...
  public ThemeComponentDto createTheme(CreateThemeRequest request) {
    ...
    List<ThemeComponent> existingThemes = themeComponentRepository.findByUserEmail(
            request.getUserEmail()).stream()
      ...
      for (ThemeStyleRequest styleRequest : request.getStyles()) {
        ColorStyle colorStyle = colorStyleRepository.findById(styleRequest.getColorTypeId())
            .orElseThrow(() -> new ResourceNotFoundException(
                "ColorStyle not found with id: " + styleRequest.getColorTypeId()));
  ...
  public List<ThemeComponentDto> getCompletedThemesByUser(String userEmail) {
    List<ThemeComponent> completedThemes = themeComponentRepository.findByUserEmailAndIsDoneTrue(
        userEmail);
    return completedThemes.stream()
        .map(this::convertToDto)
        .collect(Collectors.toList());
  ...
  private ThemeComponentDto convertToDto(ThemeComponent themeComponent) {
    List<ThemeStyle> styles = themeStyleRepository.findByThemeComponentId(
        themeComponent.getThemeComponentId());
    List<ThemeImage> images = themeImageRepository.findByThemeComponentId(
        themeComponent.getThemeComponentId());
