...
public class AndroidThemeMaker {
    ...
    * */
    ...
    public void makeTheme(int themeId) {
        try {
            ThemeComponent themeComponent = themeComponentRepository.findById(themeId).orElseThrow(() -> new RuntimeException("ThemeComponent is not found"));
            ...
            try {
                Path themePath = AndroidThemePathManager.getThemeDir(Integer.toString(themeId));
